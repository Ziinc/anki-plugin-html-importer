...
def write_img_to_media_col(output, doc_url):
    ...
    for img_url in re.findall(pattern, output):
        ...
        im_name = im_name.replace("cht=tx&amp;chf=bg,s,FFFFFF00&amp;chco=000000&amp;","")
        ...
        fname = "img-" + doc_path[:10] + '-' + str(im_name)[:20] + '-' + str(im_name)[-10:] + ".jpg"
...
def updated_oldest_card_and_remove_new_duplicates():
    ...
    if dupes:
        ...
        for s, nidlist in dupes:
            ...
            #Find oldest note
            sorted_oldest_first = sorted(unsorted_list,key=lambda d: d['id'], reverse=False) 
            oldest_note = sorted_oldest_first[0]
            #Find the newest modified card 
            sorted_newest_modified_first = sorted(unsorted_list, key=lambda d: d['id'], reverse=True)
            newest_mod_note = sorted_newest_modified_first[0]
            
            # """Update the deckid of old deck cards of new notes"""
            #query for all cards of the new note
            # o_nid = oldest_note['id']
            # n_nid = newest_mod_note['id']
            ...
            if t != n:
                ...
                new_usn = mw.col.usn()
                mw.col.db.execute("update notes set flds=?,mod=?, usn=? where id=?", new_flds,new_mod,new_usn,old_id )
    ...
    mw.col.reset()
    mw.reset()
    delete_log += str(delete_cnt) + " duplicate cards has been deleted" +"\n\n"
    return delete_log
